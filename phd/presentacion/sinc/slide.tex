...
\subsection{}
\frame{
  \frametitle{Reconocimiento automático del habla}
  \vspace{0.5cm}
  \only<1>{
    \begin{center}
...
\frame{
\frametitle{Estructuración del habla y rasgos prosódicos}
\begin{center}
\only<1>{
...
\vspace{0.2cm}
\item Duración del núcleo vocálico 
\vspace{0.2cm}
\item y otras ...
\end{itemize}
...
\begin{footnotesize}*La sensación auditiva de la $F_{0}$ es el tono de la voz o entonación.\end{footnotesize}
...
\frame{
\frametitle{Motivación: rasgos prosódicos}
% \vspace{0.4cm}
Comparación: \textbf{papá} y \textbf{capa}
\begin{flushleft}
\only<1>{
...
combinación de rasgos prosódicos ($F_0$, energía, duración temporal del núcleo vocálico, etc.) y la acentuación según las reglas ortográficas españolas.
\vspace*{0.3cm}

\textcolor{blue}{[Chen et al., 2003]}: HMM para modelar explícitamente la duración. Reetiquetando palabras y fonemas que estén al inicio y al final de frase generan un nuevo modelo de lenguaje. 
\vspace*{0.3cm}

\textcolor{blue}{[Szaszák and Vicsi, 2007]}:.
HMM entrenados con rasgos prosódicos, usado para segmentar unidades prosódicas. El sistema de RAH utiliza una red de palabras adicional ajustada en base a la salida del segmentador prosódico. 
\vspace*{0.3cm}

\textcolor{blue}{[Ananthakrishnan and \upshape{N}arayanan, 2007]}: 
clasificador binario de acentuación silábico en base a la prosodia, 
luego las secuencias acentuales de una palabra son comparadas con las de la acentuación ToBI. 
Ésto en un RAH es estándar se usa para refinar las $N$-best. %$N$ hipótesis más pobables ($N$-best). 
\vspace*{0.3cm} 

\textcolor{blue}{[Huang and Renals, 2008]}: 
se categorizan las características prosódicas a nivel de sílabas con cuantización vectorial y las palabras 
son concatenación de éstas. 
...
% % % % %   ->>>> Metodo propuesto
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % 
\section{Método propuesto}%{Trabajos previos}
\subsection{}
\frame{
...
%     \usebeamerfont{title}Método para detección de objetos]{Método para detección y seguimiento\\ de objetos con aplicaciones en Realidad Aumentada\\
%   \end{beamercolorbox}
% \vs
% \footnotesize \emph{Christian Nicolás Pfarher}\\
